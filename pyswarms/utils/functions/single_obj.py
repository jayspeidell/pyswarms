...
def crossintray_func(x):
    ...
    j = -0.0001 * np.power(
        np.abs(
            np.sin(x_) * np.sin(y_) * np.exp(
                np.abs(
                    100 - (np.sqrt(x_ ** 2 + y_ ** 2) / np.pi)
                )
            )
        ) + 1, 0.1
    )
...
def easom_func(x):
    ...
    j = -1 * np.cos(x_) * np.cos(y_) * np.exp(
        -1 * (
            (x_ - np.pi) ** 2 + (y_ - np.pi) **2
        )
    )
...
def holdertable_func(x):
    ...
    j = - np.abs(
        np.sin(x_) * np.cos(y_) * np.exp(
            np.abs(1 - np.sqrt(x_ **2 + y_ ** 2) / np.pi)
        )
    )
